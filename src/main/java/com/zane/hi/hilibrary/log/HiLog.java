...
public class HiLog {
    ...
    public static void log(@NotNull HiLogConfig config, @HiLogType.TYPE int type, @NotNull String tag, Object... contents) {
        ...
        if (config.includeThread()) {
            String format = HiLogConfig.HI_THREAD_FORMATTER.format(Thread.currentThread());
            sb.append(format).append("\n");
        }
        if (config.stackTraceDepth() > 0) {
            String format = HiLogConfig.HI_STACK_TRACE_FORMATTER.format(HiStackTraceUtil.getCroppedRealStackTrack(new Throwable().getStackTrace(), HI_LOG_PACKAGE, config.stackTraceDepth()));
            sb.append(format).append("\n");
        }
